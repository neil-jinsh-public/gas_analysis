...
def db_conn():
    ...
    try:
        ...
        engine = create_engine(f"mysql+pymysql://root:{password}@localhost:3306/zj_csaq_3304?charset=utf8mb4")
...
def get_gas_cylinders(conn):
    ...
    while True:
        query = f"SELECT * FROM zj_csaq_3304.pzrq_gas_cylinder LIMIT {batch_size} OFFSET {offset}"
...
def get_gas_status(conn):
    ...
    while True:
        query = f"SELECT * FROM zj_csaq_3304.t_gas_status LIMIT {batch_size} OFFSET {offset}"
        df_chunk = pd.read_sql_query(query, conn)
...
def get_gas_scanning(conn):
    ...
    while True:
        query = f"SELECT * FROM zj_csaq_3304.t_gas_bussiness_log LIMIT {batch_size} OFFSET {offset}"
...
def get_employee(conn):
    ...
    while True:
        query = f"SELECT * FROM zj_csaq_3304.pzrq_gas_employee LIMIT {batch_size} OFFSET {offset}"
        df_chunk = pd.read_sql_query(query, conn)
...
def get_gas_company(conn):
    ...
    while True:
        query = f"SELECT * FROM zj_csaq_3304.pzrq_gas_company LIMIT {batch_size} OFFSET {offset}"
        df_chunk = pd.read_sql_query(query, conn)
...
def get_customer(conn):
    ...
    while True:
        query = f"SELECT * FROM zj_csaq_3304.pzrq_gas_customer LIMIT {batch_size} OFFSET {offset}"
        df_chunk = pd.read_sql_query(query, conn)
