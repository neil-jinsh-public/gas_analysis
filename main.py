...
from circulation_model import (extract_status_paths, detect_abnormal_status_paths, extract_abnormal_cylinders_in_use,
                               ...
                               statics_company_fake_data_exception, statics_employee_fake_data_exception, detect_overdue_delivery, detect_user_recycled_long_time)
...
if __name__ == "__main__":
    ...
    # df_cylinder = df_cylinder[df_cylinder['cscode']=='330700']
    # df_scanning = df_scanning[
    #     (df_scanning['cscode'] == '330700') &
    #     (df_scanning['request_status'] == 1) &
    #     (df_scanning['opTime'] > analysis_time) &
    #     (df_scanning['opTime'] < analysis_time + relativedelta(months=1))]
    # df_employee = df_employee[df_employee['cscode']=='330700']
    # df_company = df_company[df_company['cscode'] == '330700']
    ### 以各地市为统计对象进行气瓶底数统计
    # 企业id与企业name对应关系
    ...
    # plot_abnormal_distribution(company_exception, employee_exception, df_cylinder, company_mapping, abnormal_distribution)
    print(15)
    # 统计企业假数据（存在同一时间的环节）异常次数、员工假数据异常次数
    # fake_data_cylinder, company_fake_data_exception = statics_company_fake_data_exception(fake_data, df_cylinder, company_mapping)
    # employee_fake_data_exception = statics_employee_fake_data_exception(fake_data, df_employee, company_mapping)
    # plot_fake_data(company_fake_data_exception, employee_fake_data_exception, df_cylinder, fake_data_cylinder, company_mapping)
    print(16)
    ...
    # plot_no_delivered_specified_time(no_delivered_specified_time, df_cylinder, company_mapping)
    print(18)
    # 获取存在7(配送入户)、8（回收）状态的数据，判断入户太久未回收的气瓶情况
    filtered_df = status_path_df[status_path_df['status_path'].apply(
        lambda path: '7' in path and '8' in path)]
    user_recycled_long_time = detect_user_recycled_long_time(filtered_df, df_customer)
    plot_user_recycled_long_time(user_recycled_long_time, df_cylinder, company_mapping)
